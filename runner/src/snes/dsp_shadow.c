...
void dsp_shadow_process(DspShadow* sh, Dsp* dsp, int canonL, int canonR,
  ...
  // Re-render the dry voice mix with cubic (vs hardware Gaussian)
  // interpolation, in float, applying the SAME envelope gain, channel
  // volumes, and master volume the canon path used. The verifier's auto-gain
  // absorbs any constant scale difference, so only structure must match.
  ...
  // Differential self-check vs the canon dry mix (same domain). Both
  // normalized to [-1,1] by the 16-bit full scale.
  ...
  if (shadow_verifier_proven(&sh->vf)) {
