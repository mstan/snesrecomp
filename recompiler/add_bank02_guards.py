#!/usr/bin/env python3
"""
Converts smw_02.c to use per-function RECOMP_BANK02 guards.
Idempotent: strips all existing RECOMP_BANK02 guards first, then re-adds.
Only func'd functions (those with `func` entries in bank02.cfg) get wrapped.
Skip'd functions and static data remain always-compiled.

Usage: python tools/recomp/add_bank02_guards.py
"""
import re

BANK_CFG  = 'tools/recomp/bank02.cfg'
ORACLE    = 'src/smw_02.c'
GUARD     = 'RECOMP_BANK02'

# --- Step 1: collect func'd names from bank02.cfg --------------------
func_names = set()
...
# --- Step 2: read smw_02.c -------------------------------------------
with open(ORACLE, 'r') as f:
    raw = f.readlines()

# --- Step 3: strip ALL existing RECOMP_BANK02 guard lines (idempotent)
stripped = []
for line in raw:
    s = line.strip()
