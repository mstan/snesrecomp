#!/usr/bin/env python3
"""
Converts smw_05.c to use per-function RECOMP_BANK05 guards.
Idempotent: strips all existing RECOMP_BANK05 guards first, then re-adds.
Only func'd functions (those with `func` entries in bank05.cfg) get wrapped.
Skip'd functions and static data remain always-compiled.

Usage: python tools/recomp/add_bank05_guards.py
"""
import re

BANK_CFG  = 'tools/recomp/bank05.cfg'
ORACLE    = 'src/smw_05.c'
GUARD     = 'RECOMP_BANK05'

# --- Step 1: collect func'd names from bank05.cfg --------------------
func_names = set()
...
# --- Step 2: read smw_05.c -------------------------------------------
with open(ORACLE, 'r') as f:
    raw = f.readlines()

# --- Step 3: strip ALL existing RECOMP_BANK05 guard lines (idempotent)
stripped = []
for line in raw:
    s = line.strip()
...
FUNC_START_RE = re.compile(r'^(?:static\s+)?(?:void|uint8|uint16|uint32|int|bool)\s+(\w+)\s*\(')
